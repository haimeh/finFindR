...
//function needs to be told 3 things:
                             ...
                             const int proximity)
{
  ...
  std::cout <<"begining path search"<<std::endl;
  while(!node_heap.empty())
  {
    ...
    if(condition(node_heap.front().get_xPos(),node_heap.front().get_yPos()) != 1 ||
       ...
       std::sqrt(std::pow(std::abs(midX-node_heap.front().get_xPos()),2)+
                 std::pow(std::abs(midY-node_heap.front().get_yPos()),2)) > maxRadius)
    {
      std::cout <<"pop: "<<node_heap.size()
                <<" ~ rad: "<<(std::sqrt(std::pow(std::abs(midX-node_heap.front().get_xPos()),2)+
                               std::pow(std::abs(midY-node_heap.front().get_yPos()),2)) > maxRadius)
                <<" ~ searched: "<<(condition(node_heap.front().get_xPos(),node_heap.front().get_yPos()) != 1)
                <<" ~ x:"<<(node_heap.front().get_xPos()>maxX)<<"->"<<(node_heap.front().get_xPos()<minX)
                <<" ~ y:"<<(node_heap.front().get_yPos()>maxY)<<"->"<<(node_heap.front().get_yPos()<minY)<<std::endl;
      ...
      {
        std::cout <<"finished status: "<<node_heap.size()<<"  "<<x<<","<<y<<std::endl;
      ...
      for(int k=0; k<8; k++)
      {
        ...
        if(newy > minY && newy < maxY && newx > minX && newx < maxX && condition(newx,newy) < 1 &&
           std::sqrt(std::pow(std::abs(midX-newx),2)+std::pow(std::abs(midY-newy),2)) < maxRadius
           )
        {
          //NEW NODE
  ...
  std::cout <<"failue to find complete path"<<std::endl;
  x=minR_x; y=minR_y;
